...
layui.define(['laytpl', 'laypage', 'layer', 'form', 'util','laydate'], function(exports){
  ...
  Class.prototype.events = function(){
    ...
    //处理子表编辑中数据字典和日期点击其他地方不消失
    //$("body").on('click',function(e){
    //layui.stope(e);
    //点击text时时间和数据字典选择框都要移除
    //$(".easyui_date_box").remove();
    //$(".data_dictionary_box").remove();
    //})
    ...
    that.layBody.on('click', 'td', function(e){
      ...
      if(editType === 'text'){
        ...
        $(".data_dictionary_box").remove();
        layui.stope(e);
      }else if(editType === 'num'){
        var input = $('<input type="number" lay-verify="number" class="layui-input edit_num '+ ELEM_EDIT +'">');
        input[0].value = othis.data('content') || elemCell.text();
        othis.find('.'+ELEM_EDIT)[0] || othis.append(input);
        input.focus();
        //点击text时时间和数据字典选择框都要移除
      ...
      }else if(editType === 'dataDictionary'){
        //我加的我加的我加的我加的我加的我加的我
        //debugger;
        var ptr = othis.parents('tr');
        var tsattdk = othis[0].attributes['data-key'];
        var tsattdkv = tsattdk.value;
        var div_cell = $(ptr[0]).find('.layui-table-cell.laytable-cell-'+ tsattdkv);
        var thisData = div_cell.text();
        // var thisEnable;
        // options.cols.forEach(function (e) {
        //   e.forEach(function (e1) {
        //     if(e1.field === field){
      ...
      }else if(editType === 'dataTree'){
        //我加的我加的我加的我加的我加的我加的我加的我加的
        singleInstance.dataTree({
          data:thisData,
          element:thisTd,
          callback:function (res) {
            layui.event.call(thisTd, MOD_NAME, 'edit('+ filter +')', commonMember.call(thisTd, {
...
//  }
  ...
  table.checkStatus = function(id){
    ...
    layui.each(data, function(i, item){
      ...
      if(item[table.config.checkName]){
        nums++;
        var item_ = table.clearCacheKey(item);
        item_.data_index__ = data_index__;
        arr.push(item_);
      }
    });
    return {
  ...
  //获取单元格值
  var getCustomGridData = function(obj){
    var ptr = $(obj).parents('tr');
    var tsattdk = obj.attributes['data-key'];
    var tsattdkv = tsattdk.value;
    var div_cell = $(ptr[0]).find('.layui-table-cell.laytable-cell-'+ tsattdkv);
    var thisData = div_cell.text();
    return  thisData;
  ...
  TableEdit.prototype.date = function(options){
    ...
    othis.deleteDate();
    othis.deleteDataDictionary();
    //获取当前值用于显示
    var thisData = getCustomGridData(that);
    var input = $('<input class="layui-input layui-table-select-input" type="text" id="thisDate" value="'+thisData+'">');
    $(that).append(input);
  ...
  //我加的我加的我加的我加的我加的我加的我加的我加的我
  //数据字典部分
  TableEdit.prototype.dataDictionary = function(options){
    var othis = this;
    othis.deleteDate();
    othis.cacheOptions = options;
    othis.callback = options.callback;
    othis.cacheOptions.dateType =  "dataDictionary";
    var that = options.element;
    othis.td = that;
    if ($(that).find('input').length>0) {
      return;
    }
    //在点击的时候先移除所有的div和input
    othis.deleteDataDictionary();
    //dom对象转为jquery对象
    var can_edit = true;
    //获取当前值用于显示
    var thisData = getCustomGridData(that);
    let gridtr = that.parentNode;
    let gridtable = gridtr.parentNode.parentNode.parentNode.parentNode.parentNode;
    let uipstr ='UI_PARAM_'+ $(gridtable).attr('lay-id')+'_'+ $(that).attr('data-field');
    let uip = eval(uipstr);
    if(isNull(uip)){
      let uipd = decodeURI(uip);
      let jo = JSON.parse(uipd);
      if(jo.paramIsEidt=='否'){
        can_edit =false;
      }
    }
    var div;
    if(can_edit){
      div = $('<div class="data_dictionary_box"><input type="text" value="'+thisData+'"><span class="data_dictionary_select"></span></div>');
    }else{
      div = $('<div class="data_dictionary_box"><input readonly="readonly" type="text" value="'+thisData+'"><span class="data_dictionary_select"></span></div>');
    }
    $(that).append(div);
    ...
    //允许数据字典单元格可编辑
    div.bind('change',function (e) {
      var newVal = e.target.value;
      //div.find('input').val(newVal);
      //console.log("newVal======"+newVal);
      options.value = newVal;
      othis.update(options);
      // debugger; //更新缓存使之可以保存
      let dtfield = that.attributes['data-field'].value;
      let gridtr = that.parentNode;
      let gridtable = gridtr.parentNode.parentNode.parentNode.parentNode.parentNode;
      let dataIndex = gridtr.attributes['data-index'].value;
      let tableId = gridtable.attributes['lay-id'].value;
      table.cache[tableId][dataIndex][dtfield]=newVal;
      table.reload(tableId,{url:'',limit:Number.MAX_VALUE,data:table.cache[tableId]});
      layui.stope();
    });
    othis.btnclick();
  }
  //给数据字典按钮绑定事件
  TableEdit.prototype.btnclick = function(){
    var othis = this;
    var current_data_key = $(othis.cacheOptions.element).attr('data-key');
    // 当前table角标
    var tableIndex = current_data_key.substring(0,1);
    // 通过当前的data-key找到相对应的th
    var current_th = $(".layui-table-header th[data-key="+current_data_key+"]");
    // 当前行
    var current_row_index = $(othis.td).parents('tr').attr('data-index');
    // 当前字段
    var filedName = current_th.attr('data-field');
    $('.data_dictionary_box').find('.data_dictionary_select').bind('click',function (e) {
      openDataDictionary(parseInt(tableIndex)-1,filedName,current_row_index,current_data_key);
    });
  }


  //数据tree
  TableEdit.prototype.dataTree = function(options){
    var othis = this;
    othis.deleteDate();
    othis.cacheOptions = options;
    othis.callback = options.callback;
    othis.cacheOptions.dateType =  "dataTree";
    var that = options.element;
    othis.td = that;
    //othis.td = that;
    if ($(that).find('input').length>0) {
    ...
    othis.deleteDataDictionary();
    //dom对象转为jquery对象
    //var can_edit = $(options.element).attr('inputBox');
    //console.log($(options.element).attr('data-field'));//点击的当前元素
    //if(can_edit){
    var div = $('<div class="data_dictionary_box"><input type="text"><span class="data_tree_select"></span></div>');
    //}else{
    //	var div = $('<div class="data_dictionary_box"><input readonly="readonly" type="text"><span class="data_tree_select"></span></div>');
    //}
    $(that).append(div);
    div.bind('click',function () {
      layui.stope();
    ...
    // 当前字段
    var filedName = current_th.attr('data-field');
    $('.data_dictionary_box').find('.data_tree_select').bind('click',function (e) {
      openComponentTree(parseInt(tableIndex)-1,filedName,current_row_index,current_data_key);
    });
  }


  //file 子表附件
  TableEdit.prototype.file = function(options){
    var othis = this;
    othis.cacheOptions = options;
    othis.callback = options.callback;
    othis.td = options.element;
    if($(othis.td).find('.layui-upload-file').length>0){
      return;
    }
    var current_data_key = $(othis.td).attr('data-key');
    // 当前table角标
    var tableIndex = current_data_key.substring(0,1);
    // 通过当前的data-key找到相对应的th
    var current_th = $(".layui-table-header th[data-key="+current_data_key+"]");
    ...
    var idValue = 'grid_file_'+tableIndex+'_'+current_row_index+'_'+filedName;
    $($(othis.td).find('.table_file_span')[0]).attr('id',idValue)
//    	$(othis.td).empty().append($div);
    renderFileUpload('#'+idValue,parseInt(tableIndex)-1,filedName,current_row_index);
  }


  //判断是否为空函数
  TableEdit.prototype.isEmpty = function(dataStr){
  ...
  //删除td中的input和时间选择器
  TableEdit.prototype.deleteDate = function(){
    $("#thisDate").next().remove();
    $("#thisDate").remove();
    $("div.layui-laydate").remove();
  };
  //删除数据字典中的所有input和div还有btn
  TableEdit.prototype.deleteDataDictionary = function(){
    $(".data_dictionary_box").remove();
  };
  //生成下拉框函数入口
  TableEdit.prototype.select = function(options){
    var othis = this;
    othis.cacheOptions = options;
    othis.callback = options.callback;
  ...
  //给下拉列表注册点击事件
  TableEdit.prototype.ddClick = function(){
    var othis = this;
  ...
  //给下拉列表注册点击事件
  TableEdit.prototype.liClick = function(){
  ...
  //生成下拉选择框的html代码(单选)
  TableEdit.prototype.createHtml = function(selectData,data){
    if(!data)data = [];
  ...
  //生成根据关键字搜索的下拉选择框(单选)
  TableEdit.prototype.searchHtml = function(selectData,search,data){
    if(!data)data = [];
    selectData.forEach(function (e) {
  ...
  //生成下拉选择框的html代码(多选)
  TableEdit.prototype.createHtmlLi = function(selectData,data){
    if(!data)data = [];
    selectData.forEach(function (e) {
  ...
  //删除所有删除下拉框和input和div
  TableEdit.prototype.deleteAll = function(td){
    var othis = this;
    othis.deleteDate();
    othis.deleteDataDictionary();
  ...
  //生成多选下拉框
  TableEdit.prototype.createDivli = function(domArr,tdInfo,width,left,type){
    var othis = this;
  ...
  //给下拉框注册鼠标悬停事件
  TableEdit.prototype.registerHover = function () {
    var othis = this;
